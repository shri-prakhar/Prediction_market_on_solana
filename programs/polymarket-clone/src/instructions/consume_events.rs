...
use crate::{
    constants::{CRANKER_REWARD_BPS, EVRNT_QUEUE_SEED, MARKET_SEED, PRICE_PRICISION_SCALE},
    error::MarketError,
    state::{vault, EventQueue, EventType, Market, OutcomeSide},
};
...
pub fn consume_events_handler<'info>(
    ...
    for event in queue.events.iter() {
        let maker_usdc = &rem[rem_index];
        rem_index += 1;
        let maker_outcome = &rem[rem_index];
        rem_index += 1;
        let taker_usdc = &rem[rem_index];
        rem_index += 1;
        let taker_outcome = &rem[rem_index];
        rem_index += 1;
        let maker_trader_position = &rem[rem_index];
        rem_index += 1;
        let taker_trader_position = &rem[rem_index];
        rem_index += 1;

        let maker_usdc_ac: InterfaceAccount<'info, TokenAccount> =
            InterfaceAccount::try_from(maker_usdc)?;
        let maker_outcome_ac: InterfaceAccount<'info, TokenAccount> =
            InterfaceAccount::try_from(maker_outcome)?;
        let taker_usdc_ac: InterfaceAccount<'info, TokenAccount> =
            InterfaceAccount::try_from(taker_usdc)?;
        let taker_outcome_ac: InterfaceAccount<'info, TokenAccount> =
            InterfaceAccount::try_from(taker_outcome)?;

        match event.event_type {
            EventType::Fill => {
                let seeds: &[&[&[u8]]] = &[seeds];
                let usdc_amount = (event.price as u128)
                ...
                let amount_after_fee =
                    usdc_amount.checked_sub(fee).ok_or(MarketError::MathError)?;

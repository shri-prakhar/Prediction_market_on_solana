...
use crate::{constants::{ASKS_SEEDS, BIDS_SEED, EVENT_QUEUE_SEED, REQUEST_QUEUE_SEED}, error::MarketError, state::{Event, EventQueue, EventType, Market, OrderSide, RequestQueue, RequestType, Slab}, utils::{dequeue_requests, find_best_price_node_index, pop_order_from_prices, push_event}};
...
pub fn match_order_handler(ctx: Context<MatchRequest> , max_requests:u16) -> Result<()>{
    ...
    for req in req.into_iter() {
        ...
        if head_entry_index == -1 { break; }
        let entry = opposite_slab.order_entries[head_entry_index as usize];
        let matched_quantity = if entry.quantity > left_quantity { left_quantity } else { entry.quantity };
        ...
        if entry.quantity > matched_quantity {
            opposite_slab.order_entries[head_entry_index as usize].quantity = entry.quantity - matched_quantity;
        }else{
            pop_order_from_prices(opposite_slab, best_price_index)?;            
        }

        left_quantity = left_quantity.checked_sub(matched_quantity).ok_or(MarketError::MathError)?;
        }

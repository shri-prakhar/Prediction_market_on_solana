...
pub fn amm_cost_to_yes(q_yes : u128 , q_no : u128 , b: u128 , delta_q: u128) -> Result<u128>{
    let mut c1 = lsmr_cost(q_yes.checked_add(delta_q).ok_or(MarketError::MathError)? , q_no , b)?;
...
pub fn amm_cost_to_no(q_yes : u128 , q_no : u128 , b: u128 , delta_q: u128) -> Result<u128>{
    let mut c1 = lsmr_cost(q_yes , q_no.checked_add(delta_q).ok_or(MarketError::MathError)? , b)?;
    let c0 = lsmr_cost(q_yes , q_no , b)?;
    c1 = c1.checked_sub(c0).ok_or(MarketError::MathError)?;
    Ok(c1)
}
...
pub fn lsmr_cost(q_yes : u128 , q_no : u128, b: u128) -> Result<u128> {
    if b == 0 {  return err!(MarketError::InvalidArgument); }
    
    let x_yes = q_yes.checked_mul(FP_SCALE).ok_or(MarketError::MathError)?.checked_div(b).ok_or(MarketError::MathError)?;
    let x_no = q_no.checked_mul(FP_SCALE).ok_or(MarketError::MathError)?.checked_div(b).ok_or(MarketError::MathError)?;
...
pub fn exp_approx(x_fp:u128) -> Result<u128> {
    ...
    let x2 = x1.checked_mul(x1).ok_or(MarketError::MathError)?.checked_div(FP_SCALE).ok_or(MarketError::MathError)?;
...
pub fn ln_approx(y_fp:u128) -> Result<u128> {
    ...
    let z_fp = numerator.checked_mul(FP_SCALE).ok_or(MarketError::MathError)?.checked_div(denominator).ok_or(MarketError::MathError)?;

    let z1 = z_fp;
    let z2 = z1.checked_mul(z1).ok_or(MarketError::MathError)?.checked_div(FP_SCALE).ok_or(MarketError::MathError)?;
    let z3 = z2.checked_mul(z1).ok_or(MarketError::MathError)?.checked_div(FP_SCALE).ok_or(MarketError::MathError)?;
    let z5 = z3.checked_mul(z2).ok_or(MarketError::MathError)?.checked_div(FP_SCALE).ok_or(MarketError::MathError)?;
    
    let mut sum = z1;
    sum = sum.checked_add(z3.checked_div(3).ok_or(MarketError::MathError)?).ok_or(MarketError::MathError)?;
    sum = sum.checked_add(z5.checked_div(5).ok_or(MarketError::MathError)?).ok_or(MarketError::MathError)?;
    
    let ln_fp = sum.checked_mul(2).ok_or(MarketError::MathError)?;
    Ok(ln_fp)
}

pub fn amm_execute_buy(market_q_yes: &mut u128 , market_q_no : &mut u128, b_fp: u128 , outcome: u8 , quantity: u128) -> Result<u128>{
    if quantity == 0 { return Ok(0);}
    ...
    if outcome == OutcomeSide::Yes as u8 {
        *market_q_yes = market_q_yes.checked_add(quantity).ok_or(MarketError::MathError)?;
    }else{
        *market_q_no = market_q_no.checked_add(quantity).ok_or(MarketError::MathError)?;
    }
...
pub fn amm_price_per_token(q_yes : u128 , q_no : u128 , b: u128 , outcome: u8) -> Result<u128> {
    if b == 0 { return err!(MarketError::MathError); }
    ...
    let prob_x = exp_x.checked_mul(FP_SCALE).ok_or(MarketError::MathError)?.checked_div(sum).ok_or(MarketError::MathError)?;

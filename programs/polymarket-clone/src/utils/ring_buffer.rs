...
use crate::{constants::{MAX_EVENTS, MAX_REQUESTS}, error::MarketError, state::{Event, EventQueue, Request, RequestQueue}};
...
pub fn dequeue_requests(queue: &mut RequestQueue , n: usize ) -> Result<Vec<Request>> {
    ...
    queue.head = queue.head.checked_add(traversal_count as u64).ok_or(MarketError::MathError)?;
    queue.count = queue.count.checked_sub(traversal_count as u64).ok_or(MarketError::MathError)?;
...
pub fn peek_events(queue: &EventQueue , n: usize) -> Vec<Event> { //why not result because this code has mo chance of failing thats just returning the value without handling Capacitythe errors 
...
pub fn pop_events(queue: &mut EventQueue , n: usize) -> Result<Vec<Event>> {
    ...
    queue.head = queue.head.checked_add(events.len() as u64).ok_or(MarketError::MathError)?;
    queue.count = queue.count.checked_add(events.len() as u64).ok_or(MarketError::MathError)?;
    Ok(events)
}